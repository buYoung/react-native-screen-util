...
import type { SafeAreaInsetType,
              screenResponsiveCheckerAction,
              screenResponsiveGetterAction,
              screenResponsiveSetterAction,
              screenResponsiveState, ScreenUtilDesignSizeDefault, ScreenUtilInitilizeParams, setStateResultType} from "./type";
...
export const ScreenResponsiveStore = createStore<screenResponsiveState & screenResponsiveGetterAction & screenResponsiveCheckerAction & screenResponsiveSetterAction>()(
    // devtools(
...
function objectValueCheckIsNull<T>(value: T): boolean {
    ...
    for (let i = 0; i < freezeValues.length; i++) {
        result.push(ScreenResponsiveStore.getState().checkIfValueIsNull(freezeValues[i]));
    }
...
function _ScreenUtilInitilize(option?: ScreenUtilInitilizeParams): setStateResultType {
    if(!option) {
        option = ScreenResponsiveStore.getState().getDefaultStyle();
    }
    try {
        const width     = option.width;
        const height    = option.height;
        ScreenResponsiveStore.setState({
        ...
        return ScreenResponsiveStore.getState().setScreenSizeRatio();
    } catch (e) {
        return makeErrorResult(e);
    }
}
export type useResponsiveProviderProps = {

};
export const ResponsiveContext = createContext<typeof ScreenResponsiveStore>({
    destroy(): void {
        console.log("삭제...?");
    },
    getState(): screenResponsiveState & screenResponsiveGetterAction & screenResponsiveCheckerAction & screenResponsiveSetterAction {
        console.log("get state");
        return ScreenResponsiveStore.getState();
    },
    setState(partial: Partial<screenResponsiveState & screenResponsiveGetterAction & screenResponsiveCheckerAction & screenResponsiveSetterAction> | ((state: (screenResponsiveState & screenResponsiveGetterAction & screenResponsiveCheckerAction & screenResponsiveSetterAction)) => (Partial<screenResponsiveState & screenResponsiveGetterAction & screenResponsiveCheckerAction & screenResponsiveSetterAction> | (screenResponsiveState & screenResponsiveGetterAction & screenResponsiveCheckerAction & screenResponsiveSetterAction))) | (screenResponsiveState & screenResponsiveGetterAction & screenResponsiveCheckerAction & screenResponsiveSetterAction), replace: boolean | undefined): void {
        console.log("set state", partial(), replace);
    },
    subscribe(listener: (state: (screenResponsiveState & screenResponsiveGetterAction & screenResponsiveCheckerAction & screenResponsiveSetterAction), prevState: (screenResponsiveState & screenResponsiveGetterAction & screenResponsiveCheckerAction & screenResponsiveSetterAction)) => void): () => void {
